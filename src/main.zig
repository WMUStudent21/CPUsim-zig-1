...
const SchedulerKind = enum { ff, rr, sp, sr, hr, fb };

const SchedulerMeta = union(SchedulerKind) { ff: struct {
...
pub fn main() !void {
    ...
    if (std.mem.eql(u8, args[1], "-s")) {
        if (std.mem.eql(u8, args[2], "FF")) kind = SchedulerMeta{ .ff = .{} } else if (std.mem.eql(u8, args[2], "RR")) {
        ...
        } else if (std.mem.eql(u8, args[2], "SP")) kind = SchedulerMeta{ .sp = .{} } else if (std.mem.eql(u8, args[2], "SR")) kind = SchedulerMeta{ .sr = .{} } else if (std.mem.eql(u8, args[2], "HR")) kind = SchedulerMeta{ .hr = .{} } else if (std.mem.eql(u8, args[2], "FB")) {
            if (std.mem.eql(u8, args[3], "-q")) {
                input_idx += 2;
                output_idx += 2;
                const quant = try std.fmt.parseInt(u8, args[4], 10);
